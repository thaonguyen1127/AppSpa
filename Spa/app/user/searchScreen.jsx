...
// Static list of Hanoi districts (unchanged)
...
// Get current location (unchanged)
...
export default function SearchHome() {
  ...
  const isFromHomeScreen = useCallback(() => {
    const routes = navigationState.routes;
    const currentRoute = routes[routes.length - 1];
    const previousRoute = routes[routes.length - 2];
    return previousRoute && previousRoute.name === 'home'; // Adjust 'home' to your actual home screen route name
  }, [navigationState]);

  // Reset state when screen is focused, only if coming from home screen
  useFocusEffect(
    useCallback(() => {
      if (isFromHomeScreen()) {
        setSearchQuery('');
  ...
  const handleSearch = useCallback(async () => {
    ...
    if (!trimmedQuery) {
      setSearchResults(['Vui lòng nhập tên spa hoặc dịch vụ']);
      return;
    }
  ...
  // Filter districts with accent-insensitive search
  const filteredDistricts = hanoiDistricts.filter((item) => {
    const queryLower = districtQuery.toLowerCase();
    const nameLower = item.name.toLowerCase();
    const nameNoAccents = removeAccents(item.name).toLowerCase();
    return nameLower.includes(queryLower) || nameNoAccents.includes(queryLower);
  });
                ...
                >
                  <TextInput
                    ...
                    onChangeText={(text) => {
                      ...
                      setShowLocationDropdown(true); // Show dropdown when typing
                      debouncedSearch();
                    }}
                    editable={true}
            ...
            <View style={styles.resultItem}>
              {typeof item === 'string' ? (
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                  onPress={() => router.push(`/user/spaDetail?id=${item.id}`)}
                >
                  <Image

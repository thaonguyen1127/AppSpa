...
const HomeScreen = () => {
  ...
  const [recommendedSpas, setRecommendedSpas] = useState([]);
  ...
  const fetchData = useCallback(async (forceRefresh = false) => {
    try {
      ...
      // console.log('Số lượng dịch vụ:', servicesSnapshot.size);
      // console.log('Dữ liệu dịch vụ:', servicesSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
      const servicesList = servicesSnapshot.docs.map(doc => ({
        id: doc.id,
        name: doc.data().name || 'Danh mục không tên',
      ...
      if (!favoritesSnapshot.empty) {
        favoritesSnapshot.forEach(doc => {
          ...
          const spaId = favorite.spaId?.id || favorite.spaId;
          if (typeof spaId === 'string') {
            if (!spaFavorites[spaId]) {
              spaFavorites[spaId] = { count: 0 };
            }
            spaFavorites[spaId].count += 1;
          } else {
            // console.warn('spaId không hợp lệ trong favorites:', favorite);
          }
        });
      }
      ...
      if (!reviewsSnapshot.empty) {
        reviewsSnapshot.forEach(doc => {
          ...
          if (!spaId || typeof spaId !== 'string' || isNaN(rating)) {
            // console.warn('Dữ liệu đánh giá không hợp lệ:', review);
            return;
          }
      ...
      spasSnapshot.forEach(doc => {
        ...
        // console.log('Spa data:', { id: doc.id, ...spaData });
        const imageUrl =
        ...
        const averageRating = spaRatings[spaId]
          ...
          : 0;
        const favoriteCount = spaFavorites[spaId]?.count || 0;

        const lat = spaData.coordinates?.lat;
        const lon = spaData.coordinates?.lon;

        // Kiểm tra tọa độ hợp lệ
        if (lat && lon && typeof lat === 'number' && typeof lon === 'number' && lat !== 0 && lon !== 0) {
          spaList.push({
            id: spaId,
          ...
          // console.warn('Spa bị bỏ qua do tọa độ không hợp lệ:', { id: doc.id, ...spaData });
        }
      });

      // console.log('Danh sách spa sau xử lý:', spaList);

      // Sắp xếp top-rated spas
      spaList.sort((a, b) => b.rating - a.rating);
      setTopRatedSpas(spaList.slice(0, 5));
      // console.log('Top rated spas:', spaList.slice(0, 5));

      // Xử lý spa gần bạn hoặc spa được gợi ý
      if (location && locationPermissionStatus === 'granted') {
        ...
        // console.log('Danh sách spa gần bạn:', nearbySpaList);
        nearbySpaList.sort((a, b) => a.meters - b.meters);
        setNearbySpas(nearbySpaList);
        setRecommendedSpas([]);
      } else {
        spaList.sort((a, b) => b.favoriteCount - a.favoriteCount);
        // console.log('Danh sách spa được gợi ý:', spaList.slice(0, 5));
        setRecommendedSpas(spaList.slice(0, 3));
        setNearbySpas([]);
      }
    } catch (error) {
  ...
  // Massage services
  if (name.includes('massage')) return 'body';
  if (name.includes('nail')) return 'hand-left';
  if (name.includes('hair') || name.includes('làm tóc')) return 'cut';
  if (name.includes('phun xăm') || name.includes('thẩm mỹ')) return 'color-palette';
  if (name.includes('chăm sóc da') || name.includes('clinic')) return 'happy';
  if (name.includes('chăm sóc cơ thể') || name.includes('body')) return 'fitness';
  if (name.includes('trang điểm') || name.includes('makeup')) return 'brush';
  if (name.includes('triệt lông') || name.includes('waxing')) return 'flash';
  if (name.includes('trị liệu')) return 'medkit';
  return 'star';
};

  // Skeleton Card: load sáng tối
  const SkeletonCard = () => (
  ...
  const handleViewAllRecommended = () => {
    router.push('/user/recommendedSpas');
  };
  ...
  const handleSpaPress = (spaId) => {
    router.push(`/user/spaDetail?id=${spaId}`);
  };
    ...
    <SafeAreaView style={styles.container} edges={['left', 'right']}>
      ...
      >
        <View style={styles.whiteContent}>
          ...
          {/* Spa gần bạn hoặc spa được gợi ý */}
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <Text style={styles.sectionTitle}>
                {locationPermissionStatus === 'granted'
                  ? `Spa gần bạn`
                  : `Gợi ý Spa`}
                  {/* ? `Spa gần bạn (${nearbySpas.length})`
                  : `Spa được gợi ý (${recommendedSpas.length})`} */}
              </Text>
              <TouchableOpacity
                onPress={locationPermissionStatus === 'granted' ? handleViewAllNearby : handleViewAllRecommended}
              >
                <Text style={styles.viewAllText}>Xem thêm</Text>
              </TouchableOpacity>
            </View>
            ...
            ) : recommendedSpas.length === 0 ? (
              <View style={styles.emptyContainer}>
                <Text style={styles.emptyText}>Chưa có spa được gợi ý</Text>
              </View>
            ) : (
              <FlatList
                data={recommendedSpas}
                horizontal
                showsHorizontalScrollIndicator={false}
                renderItem={({ item }) => (
                  <TouchableOpacity
                  ...
                  >
                    ...
                    <Text style={styles.itemSubText}>❤️ {item.favoriteCount} lượt yêu thích</Text>
                  </TouchableOpacity>
                )}
                keyExtractor={(item) => item.id}
                contentContainerStyle={{ paddingRight: 15 }}
